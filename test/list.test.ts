...
Deno.test({
    name: '01 - Parsing an un-delimited list', 
    fn: () => {  
        const input_01 = " 888, 8.88 , 'æblegrød' , 'hamster' , `GOAT`   // This is a Comment"
        const parser = new Parser<LexerTokens, Tokens, UserData>( LR, PR, 'listUnDelim')
        parser.debug = false
        ...
        const matcher_06 = tree.filter( v => (v.token === 'comment' && v.matched ) ) 
...
Deno.test({
    name: '02 - Parsing a delimited list', 
    fn: () => {  
        const input_01 = "  [ 888, 8.88 , 'æblegrød' , 'hamster' , `GOAT` ] "
        const parser = new Parser<LexerTokens, Tokens, UserData>( LR, PR, 'listDelim')
        parser.debug = false
        parser.reset(input_01)
